...
public class Client extends GameApplet {
		...
		try {
			for (int index = 0; index < indexLength; index++) {
				...
				if (data != null && data.length > 0) {
					indices[cacheIndex].method234(data.length, data, fileIndex);
	...
	private static void setBounds() {
		...
		if (loggedIn) {
			gameScreenImageProducer = new ProducingGraphicsBuffer(screenAreaWidth,
					screenAreaHeight);
		}
	}
	...
	private void drawChatArea() {
		...
		if (messagePromptRaised) {
				...
				if (chatMessages[k] != null) {
					...
					if (chatType == 6
						...
						if (chatTypeView == 2 || chatTypeView == 0) {
							...
							newRegularFont.drawBasicString(chatMessages[k],
									15 + font.getTextWidth("To :" + s1), yPos
											+ yOffset, 0x800080, shadow);
							j++;
							j77++;
						}
					}
			...
			newRegularFont.drawBasicString(inputString + "*", xOffset + 12
					+ font.getTextWidth(s + ": "), 133 + yOffset,
					changeChatArea ? 0x7FA9FF : 255, shadow);
			...
			if (!constructedViewport) {
				...
				for (int i6 = 0; i6 < k2; i6++) {
					...
					if (abyte1 != null) {
						...
						objectManager.method190(l8, collisionMaps, k9, scene,
								abyte1);
					}
				}
	...
	private void buildInterfaceMenu(int i, Widget widget, int k, int l, int i1,
		...
		for (int l1 = 0; l1 < size; l1++) {
			...
			if (childInterface.type == Widget.TYPE_CONTAINER) {
				...
				if (childInterface.optionType == Widget.OPTION_OK && k >= i2
				...
				if (childInterface.optionType == Widget.OPTION_USABLE
						&& spellSelected == 0 && k >= i2 && i1 >= j2
						&& k < i2 + childInterface.width
						&& i1 < j2 + childInterface.height) {
					...
					if (childInterface.spellName.endsWith("Rush")
						...
						menuActionText[menuActionRow] = "Autocast @gre@"
								+ childInterface.spellName;
						menuActionTypes[menuActionRow] = 104;
						secondMenuAction[menuActionRow] = childInterface.id;
						menuActionRow++;
					}
					menuActionText[menuActionRow] = s + " @gre@"
							+ childInterface.spellName;
					menuActionTypes[menuActionRow] = 626;
					secondMenuAction[menuActionRow] = childInterface.id;
					menuActionRow++;
				}
				if (childInterface.optionType == Widget.OPTION_CLOSE && k >= i2
						&& i1 >= j2 && k < i2 + childInterface.width
						&& i1 < j2 + childInterface.height) {
								...
								if (childInterface.inventoryItemId[k2] > 0) {
									...
									if (itemSelected == 1
										...
										if (childInterface.id != anInt1284
												|| k2 != anInt1283) {
											menuActionText[menuActionRow] = "Use "
													+ selectedItemName
													+ " with @lre@"
													+ itemDef.name;
										...
										if ((spellUsableOn & 0x10) == 16) {
											menuActionText[menuActionRow] = spellTooltip
													+ " @lre@" + itemDef.name;
										...
										if (childInterface.hasActions) {
											for (int l3 = 4; l3 >= 3; l3--)
												if (itemDef.actions != null
													...
													menuActionText[menuActionRow] = itemDef.actions[l3]
															+ " @lre@"
															+ itemDef.name;
													if (l3 == 3)
													...
													menuActionText[menuActionRow] = "Drop @lre@"
															+ itemDef.name;
													menuActionTypes[menuActionRow] = 847;
													selectedMenuActions[menuActionRow] = itemDef.id;
													firstMenuAction[menuActionRow] = k2;
										...
										if (childInterface.usableItems) {
											menuActionText[menuActionRow] = "Use @lre@"
													+ itemDef.name;
										...
										if (childInterface.hasActions
											...
											for (int i4 = 2; i4 >= 0; i4--)
												if (itemDef.actions[i4] != null) {
													menuActionText[menuActionRow] = itemDef.actions[i4]
															+ " @lre@"
															+ itemDef.name;
													if (i4 == 0)
						...
						if (((Entity) (obj)).hitsLoopCycle[j1] > loopCycle) {
							...
							if (spriteDrawX > -1) {
								...
								smallText
										.drawText(
												0,
												Configuration.tenXHp ? String
														.valueOf(((Entity) (obj)).hitDamages[j1] * 10)
														: String.valueOf(((Entity) (obj)).hitDamages[j1] * 1),
				...
				if (anInt1249 == 0) {
					...
					if (anIntArray981[defaultText] == 4) {
						...
						Raster.setDrawingArea(334, spriteDrawX - 50,
								spriteDrawX + 50, 0);
						boldText.render(0, s, spriteDrawY + 1,
								(spriteDrawX + 50) - k4);
						boldText.render(i3, s, spriteDrawY, (spriteDrawX + 50)
								- k4);
						Raster.defaultDrawingAreaSize();
					}
	...
	private void showOtherPlayers(boolean flag) {
		...
		for (int l = 0; l < j; l++) {
			...
			if (player.aModel_1714 != null && loopCycle >= player.anInt1707
				...
				scene.method286(plane, player.y, player, player.anInt1552,
			...
			player.anInt1709 = method42(plane, player.y, player.x);
			scene.method285(plane, player.anInt1552, player.anInt1709, i1,
	...
	private void drawHeadIcon() {
		...
		calcEntityScreenPos((hintIconX - regionBaseX << 7) + anInt937,
				anInt936 * 2, (hintIconY - regionBaseY << 7) + anInt938);
	...
	private CacheArchive createArchive(int file, String displayedName,
			String name, int expectedCRC, int x) {
		...
		while (archiveBuffer == null) {
			...
			try {
				int k1 = 0;
				DataInputStream datainputstream = requestCacheIndex(name
						+ expectedCRC);
				...
				try {
					if (indices[0] != null)
						indices[0].method234(archiveBuffer.length,
								archiveBuffer, file);
				} catch (Exception _ex) {
					indices[0] = null;
				}
	...
	private void processMenuActions(int id) {
		...
		int second = secondMenuAction[id];
    ...
    case 1050:
      ...
      break;
		...
		if (action == 234) {
			boolean flag1 = doWalkTo(2, 0, 0, 0, localPlayer.pathY[0], 0, 0,
					second, localPlayer.pathX[0], false, first);
			if (!flag1)
				flag1 = doWalkTo(2, 0, 1, 0, localPlayer.pathY[0], 1, 0,
						second, localPlayer.pathX[0], false, first);
			crossX = super.saveClickX;
			crossY = super.saveClickY;
			crossType = 2;
		...
		if (action == 62 && clickObject(clicked, second, first)) {
		...
		if (action == 74) {
			...
			if (Widget.interfaceCache[second].parent == openInterfaceId)
		...
		if (action == 1062) {
			...
			clickObject(clicked, second, first);
		...
		if (action == 484 || action == 6) {
			...
			if (l1 != -1) {
				...
				String s7 = StringUtils.formatUsername(StringUtils
						.decodeBase37(StringUtils.encodeBase37(s1)));
				boolean flag9 = false;
		...
		if (action == 626) {
			...
			if (Configuration.client_debug)
				System.out.println("spellId: " + spellId + " - spellSelected: "
						+ spellSelected);
			System.out.println(second + " " + widget.selectedActionName + " "
					+ anInt1137);
			if (spellUsableOn == 16) {
				tabId = 3;
				tabAreaAltered = true;
		...
		if (action == 956 && clickObject(clicked, second, first)) {
	...
	private void tutorialIslandAreas() {
		...
		if (j >= 3072 && j <= 3118 && k >= 9492 && k <= 9535)
			onTutorialIsland = 1;
		if (onTutorialIsland == 1 && j >= 3139 && j <= 3199 && k >= 3008
				&& k <= 3062)
			onTutorialIsland = 0;
	}
			...
			if (k1 == 2 && scene.method304(plane, i1, j1, l) >= 0) {
				...
				if (itemSelected == 1) {
					...
					if ((spellUsableOn & 4) == 4) {
						menuActionText[menuActionRow] = spellTooltip + " @cya@"
								+ objectDef.name;
						...
						if (itemSelected == 1) {
							...
							for (int j3 = 4; j3 >= 0; j3--)
								if (itemDef.groundActions != null
									...
									menuActionText[menuActionRow] = itemDef.groundActions[j3]
											+ " @lre@" + itemDef.name;
									if (j3 == 0)
									...
									menuActionText[menuActionRow] = "Take @lre@"
											+ itemDef.name;
								}
							menuActionTypes[menuActionRow] = 234;
							selectedMenuActions[menuActionRow] = item.ID;
					...
					if (myPrivilege == 2 || server.equals("127.0.0.1")) {
						...
						if (inputString.equals("::rint")) {
							GameFont gameFont = new GameFont(true, "q8_full",
									titleStreamLoader);
							GameFont fonts[] = { smallText, regularText,
									boldText, gameFont };
							CacheArchive interfaces = createArchive(3,
									"interface", "interface", archiveCRCs[3],
									35);
							CacheArchive graphics = createArchive(4,
									"2d graphics", "media", archiveCRCs[4], 40);
		...
		for (int i1 = 0; i1 < 500; i1++) {
			...
			if ((j1 == 1 || j1 == 2)
				...
				if (j > k1 - 14 && j <= k1 && !s.equals(localPlayer.name)) {
					if (myPrivilege >= 1) {
						menuActionText[menuActionRow] = "Report abuse @whi@"
								+ s;
	...
	private void buildDuelorTrade(int j) {
		...
		for (int i1 = 0; i1 < 500; i1++) {
			...
			if (chatTypeView == 4 && j1 == 8
					&& (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(s))) {
				if (j > k1 - 14 && j <= k1) {
					menuActionText[menuActionRow] = "Accept challenge @whi@"
							+ s;
		...
		for (int j = 0; j < 100; j++) {
			if (chatMessages[j] != null) {
				...
				if ((k == 3 || k == 7)
					...
					textDrawingArea
							.render(0, s + ": " + chatMessages[j], l, k1);
					textDrawingArea.render(65535, s + ": " + chatMessages[j],
							l - 1, k1);
					if (++i >= 5) {
						return;
					}
				...
				if (k == 6 && privateChatMode < 2) {
					...
					textDrawingArea.render(0, "To " + s + ": "
							+ chatMessages[j], j1, 4);
					textDrawingArea.render(65535, "To " + s + ": "
	...
	private boolean doWalkTo(int i, int j, int k, int i1, int j1, int k1,
		...
		while (i4 != l3) {
			...
			if (i1 != 0) {
				...
				if (i1 < 10
						&& collisionMaps[plane].method220(k2, i2, k3, i1 - 1,
								j, j3)) {
					flag1 = true;
					break;
				}
			...
			if (entityDef.actions != null) {
				for (int l = 4; l >= 0; l--)
					if (entityDef.actions[l] != null
						...
						menuActionText[menuActionRow] = entityDef.actions[l]
								+ " @yel@" + s;
						if (l == 0)
				...
				for (int i1 = 4; i1 >= 0; i1--)
					if (entityDef.actions[i1] != null
						...
						if (entityDef.combatLevel > localPlayer.combatLevel)
							c = '\u07D0';
						menuActionText[menuActionRow] = entityDef.actions[i1]
								+ " @yel@" + s;
						if (i1 == 0)
		...
		if (itemSelected == 1) {
			...
			for (int l = 4; l >= 0; l--)
				if (atPlayerActions[l] != null) {
					menuActionText[menuActionRow] = atPlayerActions[l]
							+ " @whi@" + s;
					char c = '\0';
	...
	void startUp() {
		...
		try {
			...
			CacheArchive streamLoader_1 = createArchive(3, "interface",
					"interface", archiveCRCs[3], 35);
			CacheArchive streamLoader_2 = createArchive(4, "2d graphics",
					"media", archiveCRCs[4], 40);
			this.mediaStreamLoader = streamLoader_2;
			CacheArchive streamLoader_3 = createArchive(6, "textures",
					"textures", archiveCRCs[6], 45);
			CacheArchive streamLoader_4 = createArchive(7, "chat system",
					"wordenc", archiveCRCs[7], 50);
			createArchive(8, "sound effects", "sounds", archiveCRCs[8],
					55);
			...
			CacheArchive streamLoader_6 = createArchive(5, "update list",
			...
			CacheArchive streamLoader_5 = createArchive(8,
	...
	public void appendEmote(Entity entity) {
		...
		if (entity.gfxId != -1 && loopCycle >= entity.graphicDelay) {
			...
			Animation animation_1 = Graphic.cache[entity.gfxId].animationSequence;
			for (entity.anInt1522++; entity.currentAnimation < animation_1.anInt352
					&& entity.anInt1522 > animation_1
							.method258(entity.currentAnimation); entity.currentAnimation++)
				entity.anInt1522 -= animation_1
						.method258(entity.currentAnimation);
	...
	private void createStationaryGraphics() {
		AnimableObject class30_sub2_sub4_sub3 = (AnimableObject) incompleteAnimables
				.reverseGetFirst();
		...
		for (int childId = 0; childId < childCount; childId++) {
			...
			if (childInterface.type == Widget.TYPE_CONTAINER) {
				...
				if (childInterface.type == Widget.TYPE_INVENTORY) {
						...
						for (int column = 0; column < childInterface.width; column++) {
							if (childInterface.inventoryItemId[slot] > 0) {
								...
								if (item.stackable
									...
									name = name
											+ " x"
											+ intToKOrMilLongName(childInterface.inventoryAmounts[slot]);
								int __x = _x + column
					...
					int boxHeight = 0;
					GameFont font = regularText;
					for (String text = childInterface.defaultText; text
							.length() > 0;) {
						if (text.indexOf("%") != -1) {
							do {
	...
	private void appendPlayerUpdateMask(int mask, int index, Buffer buffer,
			Player player) {
		if ((mask & 0x400) != 0) {
			...
			player.endForceMovement = buffer.readUShortA() + loopCycle;
	...
	private void checkForGameUsages() {
		try {
			...
			SignLink.reporterror("glfc_ex " + localPlayer.x + ","
					...
					+ anInt1069 + "," + anInt1070 + "," + regionBaseX + ","
					+ regionBaseY);
			throw new RuntimeException("eek");
		}
	}
	...
	private void draw3dScreen() {
		...
		if (Configuration.clientData) {
			...
			regularText.render(textColour, "Mouse X: " + super.mouseX
					+ " , Mouse Y: " + super.mouseY, 30,
					frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
			regularText.render(textColour, "Coords: " + x + ", " + y, 45,
					frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
			regularText.render(textColour, "Client Mode: " + screenMode + "",
					60, frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
			regularText.render(textColour, "Client Resolution: " + frameWidth
					+ "x" + frameHeight, 75, frameMode == ScreenMode.FIXED ? 5
					: frameWidth - 5);
		}
		if (systemUpdateTime != 0) {
			...
			seconds %= 60;
			if (seconds < 10)
				regularText.render(0xffff00, "System update in: " + minutes
						+ ":0" + seconds, 329 + yOffset, 4);
			else
				regularText.render(0xffff00, "System update in: " + minutes
						+ ":" + seconds, 329 + yOffset, 4);
			anInt849++;
			if (anInt849 > 75) {
				anInt849 = 0;
	...
	public final void requestCrcs() {
		...
		while (archiveCRCs[8] == 0) {
			...
			try (DataInputStream in = requestCacheIndex("crc"
	...
	private void drawMinimap() {
		...
		if (hintIconDrawType != 0 && loopCycle % 20 < 10) {
			...
			if (hintIconDrawType == 2) {
				int mapX = ((hintIconX - regionBaseX) * 4 + 2) - localPlayer.x
						/ 32;
				int mapY = ((hintIconY - regionBaseY) * 4 + 2) - localPlayer.y
						/ 32;
				refreshMinimap(mapMarker, mapY, mapX);
			}
			...
			if (chatMessages[index] != null) {
				...
				if ((type == 3 || type == 7)
					...
					if (super.mouseX > 4 && super.mouseY - offSet > y - 10
						...
						int i1 = regularText.getTextWidth("From:  " + name
								+ chatMessages[index]) + 25;
						if (i1 > 450)
							i1 = 450;
						if (super.mouseX < 4 + i1) {
							...
							menuActionText[menuActionRow] = "Add ignore @whi@"
									+ name;
							menuActionTypes[menuActionRow] = 2042;
							menuActionRow++;
							menuActionText[menuActionRow] = "Add friend @whi@"
									+ name;
	...
	private void spawnObject(int longetivity, int id, int orientation,
		...
		for (SpawnedObject node = (SpawnedObject) spawns.reverseGetFirst(); node != null; node = (SpawnedObject) spawns
				.reverseGetNext()) {
			if (node.plane != plane || node.x != x || node.y != y
					|| node.group != group)
				continue;
			object = node;
			break;
	...
	private DataInputStream requestCacheIndex(String request)
			throws IOException {
		if (!Configuration.useJaggrab) {
			if (SignLink.mainapp != null) {
	...
	private void drawLoginScreen(boolean flag) {
		...
		if (loginScreenState == 0) {
			int i = c1 / 2 + 80;
			smallText.method382(0x75a9a9, c / 2,
					resourceProvider.loadingMessage, i, true);
			i = c1 / 2 - 20;
	...
	private void parseRegionPackets(Buffer stream, int j) {
		...
		if (j == 4) {
			...
			if (i5 >= 0 && l7 >= 0 && i5 < 104 && l7 < 104) {
				i5 = i5 * 128 + 64;
				l7 = l7 * 128 + 64;
				AnimableObject class30_sub2_sub4_sub3 = new AnimableObject(plane,
		...
		if (j == 117) {
			...
			if (l5 >= 0 && k8 >= 0 && l5 < 104 && k8 < 104 && j11 >= 0
				...
				j11 = j11 * 128 + 64;
				k13 = k13 * 128 + 64;
				Projectile class30_sub2_sub4_sub4 = new Projectile(
				...
				if (group == 2) {
					...
					if (objectDef.solid)
						collisionMaps[z].removeObject(orientation,
								objectDef.width, x, y, objectDef.length,
		...
		try {
			...
			if (opcode == PacketConstants.DELETE_GROUND_ITEM) {
				...
				for (SpawnedObject object = (SpawnedObject) spawns
						...
						.reverseGetNext())
					if (object.x >= localX && object.x < localX + 8
							&& object.y >= localY && object.y < localY + 8
							&& object.plane == plane)
			...
			if (opcode == PacketConstants.SHOW_PLAYER_HEAD_ON_INTERFACE) {
				...
				if (localPlayer.desc == null)
					Widget.interfaceCache[playerHeadModelId].defaultMedia = (localPlayer.anIntArray1700[0] << 25)
					...
					Widget.interfaceCache[playerHeadModelId].defaultMedia = (int) (0x12345678L + localPlayer.desc.interfaceType);
				opcode = -1;
				return true;
			}
			...
			if (opcode == 50) {
				...
				String name = StringUtils.formatUsername(StringUtils
			...
			if (opcode == 196) {
				...
				if (!invalid && onTutorialIsland == 0)
					try {
						...
						String message = ChatMessageCodec.decode(
								packetSize - 13, incoming);
						// if(l21 != 3)
			...
			if (opcode == 246) {
				...
				if (item == 65535) {
					...
					Widget.interfaceCache[widget].modelZoom = (definition.model_zoom * 100)
							/ scale;
					opcode = -1;
					return true;
				}
			...
			if (opcode == 122) {
				...
				int blue = color & 0x1f;
				Widget.interfaceCache[id].textColor = (red << 19)
						+ (green << 11) + (blue << 3);
				opcode = -1;
				return true;
			}
	...
	private void moveCameraWithPlayer() {
		...
		Model.anInt1686 = super.mouseY
				- (frameMode == ScreenMode.FIXED ? 4 : 0);
		Raster.clear();
		scene.method313(xCameraPos, yCameraPos, xCameraCurve, zCameraPos, j,
				yCameraCurve);
	...
	private void processMinimapActions() {
		...
		if (counterHover && Configuration.enableOrbs) {
			menuActionText[2] = counterOn ? "Hide @or1@ XP Drops"
					: "Show @or1@ XP Drops";
		...
		if (prayHover && Configuration.enableOrbs) {
			menuActionText[2] = prayClicked ? "Turn quick-prayers off"
