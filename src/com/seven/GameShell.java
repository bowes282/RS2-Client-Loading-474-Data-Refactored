...
public class GameShell extends Applet implements Runnable, MouseListener,
	...
	final void createClientFrame(int w, int h) {
		...
		gameFrame = new GameFrame(this, myWidth, myHeight,
				Game.frameMode == Game.ScreenMode.RESIZABLE,
				Game.frameMode == Game.ScreenMode.FULLSCREEN);
	...
	public void mouseWheelMoved(MouseWheelEvent event) {
		...
		if (mouseX > 0 && mouseX < 512 && mouseY > Game.frameHeight - 165
				&& mouseY < Game.frameHeight - 25) {
			int scrollPos = Game.anInt1089;
			scrollPos -= rotation * 30;
			if (scrollPos < 0)
				scrollPos = 0;
			if (scrollPos > Game.anInt1211 - 110)
				scrollPos = Game.anInt1211 - 110;
			if (Game.anInt1089 != scrollPos) {
				Game.anInt1089 = scrollPos;
	...
	public void handleInterfaceScrolling(MouseWheelEvent event) {
		...
		if (tabInterfaceID != -1) {
			...
			offsetX = Game.frameMode == Game.ScreenMode.FIXED ? Game.frameWidth - 218
					: (Game.frameMode == Game.ScreenMode.FIXED ? 28
							: Game.frameWidth - 197);
			offsetY = Game.frameMode == Game.ScreenMode.FIXED ? Game.frameHeight - 298
					: (Game.frameMode == Game.ScreenMode.FIXED ? 37
							: Game.frameHeight
									- (Game.frameWidth >= 1000 ? 37 : 74) - 267);
		...
		if (Game.openInterfaceId != -1) {
			...
			offsetX = Game.frameMode == Game.ScreenMode.FIXED ? 4
					: (Game.frameWidth / 2) - 356;
			offsetY = Game.frameMode == Game.ScreenMode.FIXED ? 4
					: (Game.frameHeight / 2) - 230;
	...
	void drawLoadingText(int percentage, String loadingText) {
		...
		if (shouldClearScreen) {
			...
			graphics.fillRect(0, 0, Game.frameWidth, Game.frameHeight);
			shouldClearScreen = false;
		}
		Color color = new Color(140, 17, 17);
		int y = Game.frameHeight / 2 - 18;
		graphics.setColor(color);
		graphics.drawRect(Game.frameWidth / 2 - 152, y, 304, 34);
		graphics.fillRect(Game.frameWidth / 2 - 150, y + 2, percentage * 3, 30);
		graphics.setColor(Color.black);
		graphics.fillRect((Game.frameWidth / 2 - 150) + percentage * 3, y + 2,
				300 - percentage * 3, 30);
		graphics.setFont(font);
		graphics.setColor(Color.white);
		graphics.drawString(loadingText,
				(Game.frameWidth - fontmetrics.stringWidth(loadingText)) / 2,
				y + 22);
		graphics.drawString("",
				(Game.frameWidth - fontmetrics1.stringWidth("")) / 2, y - 8);
	}
