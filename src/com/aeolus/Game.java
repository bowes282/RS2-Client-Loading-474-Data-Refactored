...
public class Game extends GameShell {
	...
	public byte[] GetMusic(int Index) {
		try {
			File Music = new File(Signlink.findcachedir() + "./Music/" + Index + ".gz");
	...
	private void drawChatArea() {
		...
		if (messagePromptRaised) {
					...
					if (chatType == 6 && (splitPrivateChat == 0 || chatTypeView == 2) && privateChatMode < 2) {
						if (chatTypeView == 2 || chatTypeView == 0) {
							...
							newRegularFont.drawBasicString(chatMessages[k],
									15 + textDrawingArea.getTextWidth("To :" + s1), yPos + yOffset, 0x800080, shadow);
							j++;
							j77++;
						}
					}
			...
			newRegularFont.drawBasicString(inputString + "*", xOffset + 12 + textDrawingArea.getTextWidth(s + ": "),
							...
							if (k >= j3 && i1 >= k3 && k < j3 + 32 && i1 < k3 + 32) {
								...
								if (class9_1.inv[k2] > 0) {
									...
									if (itemSelected == 1 && class9_1.isInventoryInterface) {
										...
										if (Configuration.showIds && (myPrivilege >= 2 && myPrivilege <= 3)) {
			...
			for (int j = -1; j < playerCount + npcCount; j++) {
				...
				if (j < playerCount) {
					...
					if (Configuration.namesAboveHeads) {
						npcScreenPos(((Entity) (obj)), ((Entity) (obj)).height + 15);
						int col = 0x0000ff;
						if (player.clanName == myPlayer.clanName)
							col = 0x00ff00;
					...
					try {
						...
						if (spriteDrawX > -1) {
							...
							if (!Configuration.hpBar554) {
								...
								cacheSprite[40] = new Sprite(Signlink.findcachedir() + "Sprites/Attack/40.png",
										hpPercent, 7);
				...
				while (flag) {
					...
					for (int l2 = 0; l2 < k; l2++)
						if (l1 + 2 > anIntArray977[l2] - anIntArray978[l2] && l1 - k2 < anIntArray977[l2] + 2
								&& skillId - j2 < anIntArray976[l2] + anIntArray979[l2]
								&& skillId + j2 > anIntArray976[l2] - anIntArray979[l2]
	...
	private void method63() {
		TemporaryObject class30_sub1 = (TemporaryObject) aClass19_1179.reverseGetFirst();
		for (; class30_sub1 != null; class30_sub1 = (TemporaryObject) aClass19_1179.reverseGetNext())
	...
	void drawLoadingText(int i, String s) {
		...
		char c1 = '\310';
		byte byte1 = 20;
		boldText.drawText(0xffffff, Configuration.clientName + " is loading - please wait...", c1 / 2 - 26 - byte1,
				c / 2);
		int j = c1 / 2 - 18 - byte1;
	...
	public void playSong(int id) {
		if (id != currentSong && Configuration.enableMusic && !lowMem && prevSong == 0) {
			nextSong = id;
			songChanging = true;
			onDemandFetcher.method558(2, nextSong);
			currentSong = id;
	...
	private void handleMusicEvents() {
		for (int i = 0; i < anInt1062; i++) {
			...
			try {
				 ...
				 */
				Buffer stream = Sounds.method241(anIntArray1241[i], anIntArray1207[i]);
	...
	private void drawFriendsListOrWelcomeScreen(RSInterface class9) {
		...
		/** Skill Tab **/
		if (j == 210) {
			class9.message = "Total XP: " + NumberFormat.getIntegerInstance().format(xpCounter);
			return;
		}
		if (j == 211) {
			if (maxStats[0] == 99) {
				class9.message = "Attack XP: " + NumberFormat.getIntegerInstance().format(currentExp[0]);
				return;
			} else {
				class9.message = "Attack XP: " + NumberFormat.getIntegerInstance().format(currentExp[0])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[0] + 1));
		...
		if (j == 212) {
			if (maxStats[2] == 99) {
				class9.message = "Strength XP: " + NumberFormat.getIntegerInstance().format(currentExp[2]);
				return;
			} else {
				class9.message = "Strength XP: " + NumberFormat.getIntegerInstance().format(currentExp[2])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[2] + 1));
		...
		if (j == 213) {
			if (maxStats[3] == 99) {
				class9.message = "Hitpoints XP: " + NumberFormat.getIntegerInstance().format(currentExp[3]);
				return;
			} else {
				class9.message = "Hitpoints XP: " + NumberFormat.getIntegerInstance().format(currentExp[3])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[3] + 1));
		...
		if (j == 214) {
			if (maxStats[1] == 99) {
				class9.message = "Defence XP: " + NumberFormat.getIntegerInstance().format(currentExp[1]);
				return;
			} else {
				class9.message = "Defence XP: " + NumberFormat.getIntegerInstance().format(currentExp[1])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[1] + 1));
		...
		if (j == 215) {
			if (maxStats[4] == 99) {
				class9.message = "Ranged XP: " + NumberFormat.getIntegerInstance().format(currentExp[4]);
				return;
			} else {
				class9.message = "Ranged XP: " + NumberFormat.getIntegerInstance().format(currentExp[4])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[4] + 1));
		...
		if (j == 216) {
			if (maxStats[5] == 99) {
				class9.message = "Prayer XP: " + NumberFormat.getIntegerInstance().format(currentExp[5]);
				return;
			} else {
				class9.message = "Prayer XP: " + NumberFormat.getIntegerInstance().format(currentExp[5])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[5] + 1));
		...
		if (j == 217) {
			if (maxStats[6] == 99) {
				class9.message = "Magic XP: " + NumberFormat.getIntegerInstance().format(currentExp[6]);
				return;
			} else {
				class9.message = "Magic XP: " + NumberFormat.getIntegerInstance().format(currentExp[6])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[6] + 1));
		...
		if (j == 218) {
			if (maxStats[7] == 99) {
				class9.message = "Cooking XP: " + NumberFormat.getIntegerInstance().format(currentExp[7]);
				return;
			} else {
				class9.message = "Cooking XP: " + NumberFormat.getIntegerInstance().format(currentExp[7])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[7] + 1));
		...
		if (j == 219) {
			if (maxStats[8] == 99) {
				class9.message = "Woodcutting XP: " + NumberFormat.getIntegerInstance().format(currentExp[8]);
				return;
			} else {
				class9.message = "Woodcutting XP: " + NumberFormat.getIntegerInstance().format(currentExp[8])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[8] + 1));
		...
		if (j == 220) {
			if (maxStats[9] == 99) {
				class9.message = "Fletching XP: " + NumberFormat.getIntegerInstance().format(currentExp[9]);
				return;
			} else {
				class9.message = "Fletching XP: " + NumberFormat.getIntegerInstance().format(currentExp[9])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[9] + 1));
		...
		if (j == 221) {
			if (maxStats[10] == 99) {
				class9.message = "Fishing XP: " + NumberFormat.getIntegerInstance().format(currentExp[10]);
				return;
			} else {
				class9.message = "Fishing XP: " + NumberFormat.getIntegerInstance().format(currentExp[10])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[10] + 1));
		...
		if (j == 222) {
			if (maxStats[11] == 99) {
				class9.message = "Firemaking XP: " + NumberFormat.getIntegerInstance().format(currentExp[11]);
				return;
			} else {
				class9.message = "Firemaking XP: " + NumberFormat.getIntegerInstance().format(currentExp[11])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[11] + 1));
		...
		if (j == 223) {
			if (maxStats[12] == 99) {
				class9.message = "Crafting XP: " + NumberFormat.getIntegerInstance().format(currentExp[12]);
				return;
			} else {
				class9.message = "Crafting XP: " + NumberFormat.getIntegerInstance().format(currentExp[12])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[12] + 1));
		...
		if (j == 224) {
			if (maxStats[13] == 99) {
				class9.message = "Smithing XP: " + NumberFormat.getIntegerInstance().format(currentExp[13]);
				return;
			} else {
				class9.message = "Smithing XP: " + NumberFormat.getIntegerInstance().format(currentExp[13])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[13] + 1));
		...
		if (j == 225) {
			if (maxStats[14] == 99) {
				class9.message = "Mining XP: " + NumberFormat.getIntegerInstance().format(currentExp[14]);
				return;
			} else {
				class9.message = "Mining XP: " + NumberFormat.getIntegerInstance().format(currentExp[14])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[14] + 1));
		...
		if (j == 226) {
			if (maxStats[15] == 99) {
				class9.message = "Herblore XP: " + NumberFormat.getIntegerInstance().format(currentExp[15]);
				return;
			} else {
				class9.message = "Herblore XP: " + NumberFormat.getIntegerInstance().format(currentExp[15])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[15] + 1));
		...
		if (j == 227) {
			if (maxStats[16] == 99) {
				class9.message = "Agility XP: " + NumberFormat.getIntegerInstance().format(currentExp[16]);
				return;
			} else {
				class9.message = "Agility XP: " + NumberFormat.getIntegerInstance().format(currentExp[16])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[16] + 1));
		...
		if (j == 228) {
			if (maxStats[17] == 99) {
				class9.message = "Thieving XP: " + NumberFormat.getIntegerInstance().format(currentExp[17]);
				return;
			} else {
				class9.message = "Thieving XP: " + NumberFormat.getIntegerInstance().format(currentExp[17])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[17] + 1));
		...
		if (j == 229) {
			if (maxStats[18] == 99) {
				class9.message = "Slayer XP: " + NumberFormat.getIntegerInstance().format(currentExp[18]);
				return;
			} else {
				class9.message = "Slayer XP: " + NumberFormat.getIntegerInstance().format(currentExp[18])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[18] + 1));
		...
		if (j == 230) {
			if (maxStats[19] == 99) {
				class9.message = "Farming XP: " + NumberFormat.getIntegerInstance().format(currentExp[19]);
				return;
			} else {
				class9.message = "Farming XP: " + NumberFormat.getIntegerInstance().format(currentExp[19])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[19] + 1));
		...
		if (j == 231) {
			if (maxStats[20] == 99) {
				class9.message = "Runecrafting XP: " + NumberFormat.getIntegerInstance().format(currentExp[20]);
				return;
			} else {
				class9.message = "Runecrafting XP: " + NumberFormat.getIntegerInstance().format(currentExp[20])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[20] + 1));
		...
		if (j == 232) {
			if (maxStats[21] == 99) {
				class9.message = "Hunter XP: " + NumberFormat.getIntegerInstance().format(currentExp[21]);
				return;
			} else {
				class9.message = "Hunter XP: " + NumberFormat.getIntegerInstance().format(currentExp[21])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[21] + 1));
		...
		if (j == 233) {
			if (maxStats[22] == 99) {
				class9.message = "Construction XP: " + NumberFormat.getIntegerInstance().format(currentExp[22]);
				return;
			} else {
				class9.message = "Construction XP: " + NumberFormat.getIntegerInstance().format(currentExp[22])
						+ "\\nNext Level at: "
						+ NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[22] + 1));
	...
	private final void minimapHovers() {
		...
		runHover = fixed
				...
				: super.mouseX >= frameWidth - 174 && super.mouseX <= frameWidth - 120 && super.mouseY >= 132
						&& super.mouseY < 165;
		counterHover = fixed
				? counterHover = super.mouseX >= 518 && super.mouseX <= 542 && super.mouseY >= 130
						&& super.mouseY <= 154
				: super.mouseX >= frameWidth - 186 && super.mouseX <= frameWidth - 158 && super.mouseY >= 41
						&& super.mouseY <= 65;
	...
	private void login(String s, String s1, boolean flag) {
		...
		try {
			...
			socketStream = new RSSocket(this, openSocket(Configuration.server_port + portOff));
			...
			if (opCode == 6) {
				loginMessage1 = Configuration.clientName + " has been updated!";
	...
	void startUp() {
		...
		try {
			...
			for (int i = 0; i < SkillConstants.skillsCount; i++) {
				skill_sprites[i] = new Sprite("xp_counter/" + (95 + i));
			}
	...
	private void showErrorScreen() {
		...
		if (loadingError) {
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading " + Configuration.clientName, 30, k);
			k += 50;
		...
		if (genericLoadingError) {
			...
			g.drawString("To play " + Configuration.clientName + " make sure you play from", 50, 100);
		...
		if (rsAlreadyLoaded) {
			...
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of " + Configuration.clientName + " already appears to be loaded", 30, l);
			l += 50;
			g.setColor(Color.white);
	...
	public void appendEmote(Entity entity) {
		...
		if (entity.anInt1520 != -1 && loopCycle >= entity.anInt1523) {
			...
			if (entity.nextGraphicsAnimationFrame >= animation_1.anInt352) {
				if (entity.nextGraphicsAnimationFrame < 0 || entity.nextGraphicsAnimationFrame >= animation_1.anInt352)
					entity.anInt1520 = -1;
			}
		}
	...
	private void drawGameScreen() {
		...
		if (invOverlayInterfaceID != -1) {
			animateRSInterface(anInt945, invOverlayInterfaceID);
		}
		drawTabArea();
		if (backDialogID == -1) {
	...
	private void createStationaryGraphics() {
		SceneSpotAnim class30_sub2_sub4_sub3 = (SceneSpotAnim) aClass19_1056.reverseGetFirst();
		for (; class30_sub2_sub4_sub3 != null; class30_sub2_sub4_sub3 = (SceneSpotAnim) aClass19_1056.reverseGetNext())
...
//					"frameWidth: " + (mouseX - frameWidth) + ", frameHeight: " + (mouseY - frameHeight),
//					frameHeight - 271, 5);
			regularText.method385(textColour, "Client Zoom: " + cameraZoom, 90, frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
			...
			regularText.method385(textColour, "Mouse X: " + super.mouseX + " , Mouse Y: " + super.mouseY, 30, frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
			regularText.method385(textColour, "Coords: " + x + ", " + y, 45, frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
			regularText.method385(textColour, "Client Mode: " + screenMode + "", 60, frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
			regularText.method385(textColour, "Client Resolution: " + frameWidth + "x" + frameHeight, 75, frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
		}
		if (time != 0) {
			int j = time / 50;
			int l = j / 60;
	...
	private void method115() {
		if (loadingStage == 2) {
			for (TemporaryObject class30_sub1 = (TemporaryObject) aClass19_1179
					.reverseGetFirst(); class30_sub1 != null; class30_sub1 = (TemporaryObject) aClass19_1179
							.reverseGetNext()) {
	...
	private void drawMinimap() {
		...
		if (anInt1021 == 2) {
			...
			if (frameMode == ScreenMode.FIXED
					...
					: super.mouseX >= frameWidth - 185 && super.mouseX <= frameWidth - 158 && super.mouseY >= 40
							&& super.mouseY <= 66) {
				// cacheSprite[23].drawSprite(frameMode == ScreenMode.FIXED ? 1
				// : frameWidth - 185, frameMode == ScreenMode.FIXED ? 46 : 41);
			} else {
				// cacheSprite[22].drawSprite(frameMode == ScreenMode.FIXED ? 1
				// : frameWidth - 185, frameMode == ScreenMode.FIXED ? 46 : 41);
			}
		...
		for (int i6 = 0; i6 < npcCount; i6++) {
			...
			if (npc != null && npc.isVisible()) {
				...
				if (entityDef != null && entityDef.drawMinimapDot && entityDef.aBoolean84) {
	...
	private void calcEntityScreenPos(int i, int j, int l) {
		...
		i1 = j2;
		if (l >= 50) {
			spriteDrawX = Texture.textureInt1 + (i << SceneGraph.viewDistance) / l;
			spriteDrawY = Texture.textureInt2 + (i1 << SceneGraph.viewDistance) / l;
		} else {
			spriteDrawX = -1;
			spriteDrawY = -1;
		}
	...
	private void method130(int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int j2) {
		...
		for (TemporaryObject class30_sub1_1 = (TemporaryObject) aClass19_1179
				.reverseGetFirst(); class30_sub1_1 != null; class30_sub1_1 = (TemporaryObject) aClass19_1179
						.reverseGetNext()) {
	...
	private void drawLoginScreen(boolean flag) {
		...
		//regularText.method385(0xffffff, "Mouse X: " + super.mouseX + " , Mouse Y: " + super.mouseY, 30, frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
		char c = '\u0168';
		char c1 = '\310';
		...
		if (loginScreenState == 0) {
			...
			i = c1 / 2 - 20;
			boldText.method382(0xffff00, c / 2, "Welcome to " + Configuration.clientName, i, true);
			i += 30;
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
		...
		if (loginScreenState == 2) {
			...
			boldText.method389(true, c / 2 - 90, 0xffffff,
					"Login: " + myUsername + ((loginScreenCursorPos == 0) & (loopCycle % 40 < 20) ? "@yel@|" : ""), j);
			j += 15;
	...
	private void method137(Buffer stream, int j) {
		...
		if (j == 4) {
			...
			if (i5 >= 0 && l7 >= 0 && i5 < 104 && l7 < 104) {
				i5 = i5 * 128 + 64;
				l7 = l7 * 128 + 64;
				SceneSpotAnim class30_sub2_sub4_sub3 = new SceneSpotAnim(plane, loopCycle, j15, k10,
		...
		if (j == 117) {
			...
			if (l5 >= 0 && k8 >= 0 && l5 < 104 && k8 < 104 && j11 >= 0 && k13 >= 0 && j11 < 104 && k13 < 104
				...
				j11 = j11 * 128 + 64;
				k13 = k13 * 128 + 64;
				SceneProjectile class30_sub2_sub4_sub4 = new SceneProjectile(i21, l18, k19 + loopCycle, j20 + loopCycle,
		...
		try {
			...
			case PacketConstants.SEND_SKILL:
				...
				if (skillId < currentExp.length) {
					int x2 = xp - currentExp[skillId];
					if (currentExp[skillId] > -1)
						addToXPCounter(skillId, x2);
					currentExp[skillId] = xp;
			...
			case PacketConstants.NEXT_OR_PREVIOUS_SONG:
				int next = inStream.method436();
				int previous = inStream.method435();
				if (Configuration.enableMusic && !lowMem) {
					nextSong = next;
					songChanging = false;
					onDemandFetcher.method558(2, nextSong);
	...
	private void processMinimapActions() {
		...
		if (hpHover) {
			menuActionName[1] = Configuration.HPaboveHeads ? "Turn HP Above Heads on" : "Turn HP Above heads off";
			menuActionID[1] = 1508;
			menuActionRow = 2;
		}
		if (prayHover) {
	...
	private void drawSpecialOrb() {
		...
		if (specialHover) {
			cacheSprite[56].drawSprite(frameMode == ScreenMode.FIXED ? 153 : frameWidth - 63,
					frameMode == ScreenMode.FIXED ? 131 : 150);
		} else {
			cacheSprite[42].drawSprite(frameMode == ScreenMode.FIXED ? 154 : frameWidth - 62,
					frameMode == ScreenMode.FIXED ? 132 : 151);
		}
		cacheSprite[5].myHeight = (int) (spec * 27 / 100);
		cacheSprite[5].drawSprite(frameMode == ScreenMode.FIXED ? 157 : frameWidth - 58,
				frameMode == ScreenMode.FIXED ? 135 : 155);
		cacheSprite[14].drawSprite(frameMode == ScreenMode.FIXED ? 157 : frameWidth - 58,
				frameMode == ScreenMode.FIXED ? 135 : 155);
		cacheSprite[55].drawSprite(frameMode == ScreenMode.FIXED ? 162 : frameWidth - 53,
				frameMode == ScreenMode.FIXED ? 140 : 160);
		smallText.method382(getOrbTextColor(spec), frameMode == ScreenMode.FIXED ? 198 : frameWidth - 19,
				Integer.toString(spec), frameMode == ScreenMode.FIXED ? 158 : 177, true);
	}
	...
	private void loadAllOrbs(int xOffset) {
		...
		for (int i = 0; i < 3; i++) {
			...
			double percent = (i == 2 ? Configuration.runEnergy ? currentEnergy / 100D : 100 : level / 100D),
					fillHp = 26 * percent, fillPrayer = 26 * percent, fillRun = 26 * percent;
			...
			if (Configuration.TenXHP) {
				smallText.method382(getOrbTextColor(i == 2 ? Configuration.runEnergy ? currentEnergy : 100 : level),
						orbTextX[i] + xOffset,
						"" + (i == 2 ? Configuration.runEnergy ? cEnergy : 100
								: i == 0 && Configuration.newDamage ? currentHP * 10
				...
				smallText.method382(getOrbTextColor(i == 2 ? Configuration.runEnergy ? currentEnergy : 100 : level),
						orbTextX[i] + xOffset,
						"" + (i == 2 ? Configuration.runEnergy ? cEnergy : 100
								: i == 0 && Configuration.newDamage ? currentHP * 1
										: RSInterface.interfaceCache[currentInterface[i]].message.replaceAll("%", "")),
						orbTextY[i], true);
			}
		}
		...
		for (i = 0; i < xp_added.length; i++) {
			if (xp_added[i][0] > -1) {
				if (xp_added[i][2] >= 0) {
					...
					String s = "<col=4682B4><trans=" + transparency + ">+"
					...
					for (int i2 = 0; i2 < skill_sprites.length; i2++) {
						...
						sprite.drawSprite(x - a + 12 - xp_font.getTextWidth(s) - icons_x_off,
								y + 150 + (140 - xp_added[i][2]) - (font_height / 2) - (sprite.myHeight / 2),
								transparency);
					}
					xp_font.drawBasicString(s, x - a + 12 - xp_font.getTextWidth(s), y + 150 + (140 - xp_added[i][2]),
							0xffffff, -1);
				}
